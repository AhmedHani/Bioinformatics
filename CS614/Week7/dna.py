...
class DNA(object):
    ...
    def get_k_mers(self, k):
        return [self.__dna_string[i:(i + k)] for i in range(0, len(self.__dna_string) - k + 1)]

    def get_most_probable_k_mer(self, k, score_matrix):
        k_mers = self.get_k_mers(k)
        most_probable_k_mer = ""
        max_score = -1.0
        ...
        for k_mer in k_mers:
            ...
            for i, char in enumerate(k_mer):
                score *= score_matrix[char][i]
